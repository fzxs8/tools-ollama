...
const loadAvailableServers = async () => {
  try {
    ...
    emit('update:availableServers', [localServer, ...remoteServers])

    try {
      const activeServer = await GetActiveServer()
      const activeServerExists = activeServer && activeServer.id && [localServer, ...remoteServers].some(s => s.id === activeServer.id)
